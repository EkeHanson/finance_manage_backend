...
class ProofOfAddressSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id", "type", "document", "document_url","nin", "issue_date", "nin_document", "nin_document_url", "last_updated_by_id", "last_updated_by"]
        read_only_fields = ["id", "last_updated_by", "last_updated_by_id"]
...
class UserProfileSerializer(serializers.ModelSerializer):
    ...
    skills = SkillDetailSerializer(many=True, required=False, allow_null=True, source='skill_details')
    ...
    def to_internal_value(self, data):
        """
        Accept both 'skills' and 'skill_details' keys from incoming payloads.
        DRF expects the input to match the serializer field name ('skills')
        even when the source maps to the related_name ('skill_details').
        """
        if isinstance(data, dict):
            # Remap 'skill_details' -> 'skills' for input compatibility
            if "skill_details" in data and "skills" not in data:
                data = dict(data)
                data["skills"] = data.pop("skill_details")
    ...
    def validate(self, data):
        ...
        for field in date_fields:
            if field in data and data[field] == '':
                data[field] = None
                logger.info(f"Converted empty string to None for {field}")
    ...
    def create(self, validated_data):
        ...
        # Accept either 'skills' or 'skill_details' in incoming data
        nested_data = {}
        for field, _, _ in nested_fields:
            if field == "skills":
                items = validated_data.pop("skills", None)
                if items is None:
                    items = validated_data.pop("skill_details", [])
                nested_data["skills"] = items if items is not None else []
            else:
        ...
        with transaction.atomic():
            ...
            for field, _, _ in nested_fields:
                if field == "skills":
                    items = validated_data.pop("skills", None)
                    if items is None:
                        items = validated_data.pop("skill_details", None)
                    nested_data["skills"] = items
                else:
                    nested_data[field] = validated_data.pop(field, None)
...
class UserCreateSerializer(serializers.ModelSerializer):
    ...
    def to_internal_value(self, data):
        ...
        # Convert empty strings to None for date fields in profile_data
        date_fields = [
            'drivers_licence_date_issue',
            'drivers_licence_expiry_date',
            'drivers_licence_insurance_expiry_date',
    ...
    def create(self, validated_data):
        ...
        with tenant_context(tenant):
            ...
            # Remap 'skill_details' -> 'skills' for input compatibility
            if isinstance(profile_data, dict) and "skill_details" in profile_data and "skills" not in profile_data:
                profile_data["skills"] = profile_data.pop("skill_details")
