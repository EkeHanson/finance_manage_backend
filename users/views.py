...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        with tenant_context(tenant):
            ...
            # Refresh instance from DB with all related data to include newly created nested objects
...
class GenericDetailView(APIView):
    ...
    def get(self, request, obj_id):
        """Retrieve an instance by ID, scoped to the user's profile."""
        try:
            user_profile = self.get_user_profile(request)
            if not user_profile:
                return Response(
            ...
            logger.info(f"{self.model_name} {obj_id} retrieved for user profile {user_profile.id}")
            return Response(
                {
                    "status": "success",
            ...
            logger.error(f"Error retrieving {self.model_name} {obj_id}: {str(e)}")
            return Response(
                {"status": "error", "message": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def delete(self, request, obj_id):
        """Delete an existing instance by ID, scoped to the user's profile."""
        try:
            user_profile = self.get_user_profile(request)
            if not user_profile:
                return Response(
            ...
            logger.info(f"{self.model_name} {obj_id} deleted for user profile {user_profile.id}")
            return Response(
                {"status": "success", "message": f"{self.model_name} deleted successfully."},
                status=status.HTTP_204_NO_CONTENT
            )
        except Exception as e:
            logger.error(f"Error deleting {self.model_name} {obj_id}: {str(e)}")
            return Response(
                {"status": "error", "message": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
class BulkUserDetailsView(APIView):
    ...
    def post(self, request):
        ...
        with tenant_context(tenant):
            ...
            user = request.user
            # if not (user.is_superuser or user.role == "co-admin"):
            #     if user.role == "team_manager":
            #         pass  # All non-client users in tenant
            #     elif user.role == "recruiter" and user.branch:
