...
class TokenValidateView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            # Check user status - same validation as CustomTokenSerializer
            ...
            if is_locked or user_status == "suspended" or user_status == "inactive" or not is_active:
                logger.warning(f"Token validation failed for user {user.email}: Account locked or suspended")
                return Response({
                    "status": "error",
...
class CustomTokenSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        # Directly generate tokens and complete login (no OTP)
        logger.info(f"✅ Login successful for user {user.email} - generating tokens directly")
        method = "username" if "username" in attrs else "email"
        tokens = self._generate_tokens(user, remember_me, method, ip_address, user_agent)
        
        # Log successful login
        self._log_activity(user, user.tenant, "login",
                          {"reason": "Login successful", "method": method},
    ...
    def _generate_tokens(self, user, remember_me, method, ip_address, user_agent):
        ...
        access_payload = {
            ...
            "role": role,
            "status": status,
            "tenant_id": user.tenant.id,  # Use integer ID for lookups
            "tenant_unique_id": str(user.tenant.unique_id),  # Keep UUID for display
        ...
        return {
            ...
            "tenant_id": user.tenant.id,  # Integer ID for API lookups
            "tenant_unique_id": str(user.tenant.unique_id),  # UUID for display
            "tenant_organizational_id": str(user.tenant.organizational_id),
            "tenant_name": str(user.tenant.name),
    ...
    def _send_login_notification(self, user, method, ip_address, user_agent, remember_me):
        ...
        try:
            ...
            event_payload = {
                ...
                "tenant_id": user.tenant.id,  # Use integer ID
                "tenant_unique_id": str(user.tenant.unique_id),  # Keep UUID for display
...
class CustomTokenObtainPairView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            # Get validated data (includes tokens directly now)
            validated_data = serializer.validated_data
