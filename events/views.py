...
class EventViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        #     return Event.objects.filter(tenant=tenant).select_related('creator').prefetch_related('participants')
        # else:
    ...
    def create(self, request, *args, **kwargs):
        """Override create to return refreshed data with all related objects."""
        serializer = self.get_serializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        self.perform_create(serializer)
        
        # Refresh the instance to get all related data
        instance = Event.objects.select_related('creator').prefetch_related('participants').get(pk=serializer.instance.pk)
    ...
    def update(self, request, *args, **kwargs):
        """Override update to return refreshed data with all related objects."""
    ...
    def my_events(self, request):
        ...
        user = request.user
        tenant = user.tenant
        events = Event.objects.filter(creator=user, tenant=tenant).select_related('creator').prefetch_related('participants')
    ...
    def my_invitations(self, request):
        ...
        events = Event.objects.filter(
            participants=user,
            tenant=tenant
        ).exclude(creator=user).distinct().select_related('creator').prefetch_related('participants')
        serializer = self.get_serializer(events, many=True)
        return Response(serializer.data)
    ...
    def add_participants(self, request, pk=None):
        ...
        # Refresh to get updated participants list
